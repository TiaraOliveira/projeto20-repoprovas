...
describe('Test discipline Routes', () => {
 

  it('Deve retornar as disciplinas', async () => {
    const body =  {
    ...
    const result = await supertest(app).get("/discipline").set({Authorization : `Bearer ${token}`}).send();
  ...
  it('Deve retornar erro ao não enviar o token', async () => {
    ...
    const result = await supertest(app).get("/discipline").set({Authorization : `Bearer `}).send();
